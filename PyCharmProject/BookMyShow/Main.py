...
def disp():
    ...
    for i in seat:
        for j in i:
            ...
            print( " ->Silver Class -250Rs" if j > 'c' else (" ->Gold Class -350Rs" if j == 'c' else " ->Platinum Class -500Rs"),end="")
        print()
...
def validate(no_seat,row,frm):
    ...
    try:
        val=seat.get(row)
        for i in range(int(frm),int(frm)+int(no_seat) or int(no_seat)+int(frm)<11):
            if(i>len(val)-1 or val[i]!=str(i)):
                return False
...
while(1):
    ...
    if(validate(no_seat,row,frm)):
        ...
        print("#####Sorry, Total number of seat is no available in this Row#####")
        continue
    disp()
    #print(bill)
